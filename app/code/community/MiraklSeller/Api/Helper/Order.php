...
class MiraklSeller_Api_Helper_Order extends MiraklSeller_Api_Helper_Client_MMP
{
     ...
     */
    public function getAllOrders(Connection $connection, array $params = array(), $sortBy = 'dateCreated', $dir = 'DESC')
    {
        $offset = 0;
        $max    = 100;
        $orders = array();
    ...
    public function getOrders(
        ...
        Mage::dispatchEvent(
            'mirakl_seller_api_get_orders_before', array(
                'request'    => $request,
                'connection' => $connection,
            )
        );
    ...
    {
        $orders = $this->getOrders($connection, array('order_ids' => $orderId));
    ...
    public function getOrderMessages(
        ...
        Mage::dispatchEvent(
            'mirakl_seller_api_get_order_messages_before', array(
                'request'      => $request,
                'mirakl_order' => $miraklOrder,
            )
        );
